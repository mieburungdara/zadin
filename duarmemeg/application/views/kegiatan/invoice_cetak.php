...
// var_dump($permohonan->id);
    ...
    var_dump($no_rkbm);
    // $no_rkbm = ltrim(explode('.', $no_rkbm[2]), 0); ?>

                                                <tr>
                                                    <th scope="row" style="text-align: center;vertical-align: middle;"><?=$naik++; ?></th>
                                                    <td colspan="9">Jasa PBM (RKBM <?=$status; ?>)<br>No RKBM : <?=$no_rkbm; ?><br>Tanggal : <?=tgl_in($permohonan->mulai); ?><br>Nama Kapal : <?=$nama_kapal; ?><br>Muatan : <?=number_format(str_replace('000', '', $permohonan->jumlah_asli), 0, ',', '.'); ?>&ensp;&ensp;&ensp;MT <?=$tarif; ?></td>
